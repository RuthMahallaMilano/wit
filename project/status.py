...
def status_function():
    ...
    changes_to_be_committed = show_files(get_changes_to_be_committed(repository, staging_area_path))
...
def get_changes_to_be_committed(repository: Path, staging_area_path: Path) -> Iterator[Path]:
    last_commit_id = get_head_reference(repository)
...
def get_changes_not_staged_for_commit(repository: Path, staging_area_path: Path) -> Iterator[Path]:
    files_in_staging_area = get_all_files_in_directory_and_subs(staging_area_path)
    for file_path in files_in_staging_area:
